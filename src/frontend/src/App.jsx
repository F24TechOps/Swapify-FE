...
function App() {
  const [company, setCompany] = useState(localStorage.getItem('selectedCompany') || 'Force24');
  ...
  const handleDeleteCompany = (comp) => {
    ...
    if (company === comp) {
      const newSelectedCompany = filteredCompanies.length > 0 ? filteredCompanies[0] : 'Force24';
  ...
  return (
    <Router>
      ...
      <button onClick={toggleCreateNewCompany}>
        ...
        <button onClick={() => handleDeleteCompany(company)}>Delete Company</button> 
      </div>
      <Routes>
        <Route path="/microsite" element={<Editor type="microsite" company={company} />} />
        <Route path="/email" element={<Editor type="email" company={company} />} />
